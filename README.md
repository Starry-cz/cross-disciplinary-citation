...
- Word：作者—年份精确映射为 REF 域；顺序编码可先检查文后编号类型，再批量生成可见书签、上标 REF 域；只有显式传入参数时才会清除 Word 自动编号。
...
- 已有 `[N]` 文本编号时，可用 `scripts/reference_auto_numbering.py` 生成 Word 自动编号版；在 Windows Microsoft Word 环境中再使用 `word_auto_refcite.ps1` 生成可点击、上标的正文编号引用。
